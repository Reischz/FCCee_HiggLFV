...
def run_analysis_81T101(file_dir, res_name):
    print(f"--> Starting job: {res_name}")
    
    # Prepare the ROOT C++ command
    config_json = "../pipeline_mue_81To101.json"
    root_cmd = f'root -l -b -q "../analyze_pipeline.cpp(\\"{file_dir}\\", \\"{res_name}_81To101Hist.root\\", \\"{config_json}\\")"'
    full_cmd = f"{env_setup}\n{root_cmd}"
    
    # Open the log file using a context manager (the 'with' block ensures it closes safely)
    with open(f"{res_name}_81To101log.txt", "w") as log_file:
        # subprocess.run blocks *this specific thread* until the ROOT script finishes
        subprocess.run(full_cmd, shell=True, stdout=log_file, stderr=subprocess.STDOUT, executable="/bin/bash")
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for f_dir, r_name in zip(BACKGROUND_DIRS, BACKGROUND_NAMES):
            future = executor.submit(run_analysis_21T81, f_dir, r_name)
            future = executor.submit(run_analysis_81T101, f_dir, r_name)
        ...
        for i in range(110, 165, 5):
            ...
            future = executor.submit(run_analysis_21T81, f_dir, res_name)
            future = executor.submit(run_analysis_81T101, f_dir, res_name)
            futures.append(future)
